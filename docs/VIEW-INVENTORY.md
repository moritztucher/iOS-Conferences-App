...
| `ConferenceDetailView` | `Features/ConferenceDetail/Views/ConferenceDetailView.swift` | ConferenceDetail | `ConferenceDetailViewModel` | List-based detail led by the stretchy `ConferenceDetailHero` (name + date overlaid, so there's no separate title section). Editorial order below: **About** (the blurb) → **When & Where**. The latter is one grouped card: a Date row with a relative **countdown** (`ConferenceDateStyle.countdown`), a **Time** row for timed events (zone abbr included), the Location row, and a **full-bleed MapKit map** embedded as the section's final row (venue `Marker`, geocoded by `VenueLocationService`, "Open in Maps" material badge, taps through to Maps) — no longer a separate floating map card. Primary actions live in a pinned `.safeAreaInset(.bottom)` bar (`.bar` material): **Website** (bordered) + **Calendar** (bordered-prominent). Nav-bar title + toolbar background fade in once the hero scrolls away. Rich `ShareLink` (`SharePreview`). |
