...
| `EventEditorView` | `Features/ConferenceDetail/Views/EventEditorView.swift` | UIViewControllerRepresentable wrapping `EKEventEditViewController` (system event editor sheet). | `event: EKEvent, eventStore: EKEventStore, onSaved: () -> Void` | Only used in ConferenceDetail today. `onSaved` fires only when the delegate completes with `.saved` (not cancel/delete) — drives the "add to calendar" icon achievement. |
...
| Component | File | Purpose | Key Inputs | Notes |
|-----------|------|---------|------------|-------|
| `IconUnlockCelebrationView` | `Features/Settings/Views/IconUnlockCelebrationView.swift` | Full-takeover sheet celebrating a freshly-earned collectible icon (ADR-0008): "Ticket Unlocked" eyebrow + sparkle, serif title, date · venue, the ticket artwork revealing with a marigold glow + shine sweep, lore, and two actions — **Use this icon** (`.glassProminent`, applies via `AppIconViewModel`) / **Keep collecting**. | `icon: AppIcon` | Presented app-wide from `RootTabView` whenever `AchievementService.pendingCelebration` is non-nil. Reveal motion + shine are reduce-motion-gated; success haptic fires once on appear; content scrolls for Dynamic Type. |
...
|--------|------|---------|-----------|-------|
| `RootTabView` | `Features/RootTabView.swift` | (root) | — | `TabView`: Conferences / Favourites / Settings + a `Tab(role: .search)` (iOS 26 search affordance in the tab bar) hosting `ConferenceSearchView`. Also the **achievement host** (ADR-0008): configures `AchievementService` with the model context in `.task` (replaying date/threshold unlocks on launch) and presents `IconUnlockCelebrationView` over everything via `.sheet(item:)` bound to the service's celebration queue. |
...
| `AppearanceView` | `Features/Settings/Views/AppearanceView.swift` | Settings | `AppIconViewModel` (+ `AchievementService` via environment) | Pushed from Settings → Display. **Color Scheme** stays a stock segmented `Picker` (persisted via `@AppStorage("settings.colorScheme")`, applied app-wide with `.preferredColorScheme` at the root). **App Icon** is the collection's vitrine: a two-column `LazyVGrid` of private `AppIconTicketPage`s, all vertically scrolling, no horizontal scroll, under a "**App Icon · N of 7 collected**" header (ADR-0008). **Unlocked** tickets show artwork + title + date · venue + `AppIcon.lore` and select on tap (the selected ticket wears a marigold ring + inset `checkmark.seal` stamp and spins once when newly chosen — flip + selection haptic both reduce-motion-gated; switching goes through `AppIconService` / `setAlternateIconName`). **Locked** tickets are desaturated + dimmed behind a `lock.fill`, the `AppIcon.unlockHint` shown as a goal, with a live marigold `ProgressView` + `n / target` for the favourite-count tickets (`AchievementService.progress(for:)`); the button is disabled. Icons are collectible Apple-history milestones earned through real use; the default ticket-fan icon is always unlocked. |
