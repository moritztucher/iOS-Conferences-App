...
| Component | File | Purpose | Key Inputs | Notes |
|-----------|------|---------|------------|-------|
| `ConferenceSectionList` | `Features/ConferenceList/Views/ConferenceSectionList.swift` | **Kind-sectioned** (Conferences → Events → Watch Parties, each sorted by date), **`.plain`** list of conferences rendered as floating full-bleed cards with a trailing favourite swipe action. Shared by the Conferences/Favourites tabs and the Search tab so every entry point renders identically. | `sections: [ConferenceListSection], favouriteIDs: Set<String>, namespace: Namespace.ID, onToggleFavourite: (Conference) -> Void` | Host owns the `NavigationStack` + `navigationDestination` and supplies the favourite toggle **and a `@Namespace`**. Each card gets `.matchedTransitionSource(id:in:)`; the host's destination applies `.navigationTransition(.zoom(sourceID:in:))`, so tapping a card zooms into the detail hero. Rows clear their background/separator/insets so `ConferenceCard` floats; navigation uses a `.background` value-based `NavigationLink`. `swipeActions` live here. Section headers use a private editorial `ListSectionHeader` (bold, tracked, primary, card-aligned, **with a trailing count** so dense kinds announce themselves). |
...
| `ConferenceListView` | `Features/ConferenceList/Views/ConferenceListView.swift` | ConferenceList | `ConferenceListViewModel` | Used by both Conferences and Favourites tabs via `init(filter:)`. Renders via `ConferenceSectionList` (kind-grouped); hosts `.refreshable`, a filter `Menu` (multi-select Kind toggles + Format + past toggle), and the `navigationDestination(for: Route.self)`. Search is no longer per-tab — it lives in the global Search tab. |
