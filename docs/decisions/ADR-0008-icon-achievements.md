...
- **Three timings — live, deferred, and silent.** An `isReplaying` flag (set during the launch replay) keeps unlocks from slamming a sheet over the first screen on cold launch. From there:
  - **Live** (action this session) → celebrate immediately, anywhere.
  - **Seasonal earned at launch** (the WWDC26 date window) → **deferred**: persisted with `UnlockedIcon.celebrationShown == false` and presented later from the **Appearance screen's** `.task` (`flushDeferredCelebrations()`), the collection's home and a calm moment — never cold launch. Marked shown so it fires exactly once, surviving a quit.
  - **Threshold an updating user already passed** → **silent forever** (`celebrationShown == true`): retroactively celebrating old activity would be a confusing burst, so it just appears collected.
...
- The WWDC26 ticket is only earnable during June 2026; after the window closes it can no longer be unlocked. This is intentional — it is a seasonal collectible tied to the app's ship date. Earning it is silent at launch; its celebration is **deferred** to the next visit to Settings → Appearance.
- The deferred celebration is tied to the persisted `celebrationShown` flag, not re-checked against the date, so it fires whenever the user next opens Appearance — **even months later** (e.g. a first visit in September). Chosen deliberately over expiring it: never lose the earned moment, standard achievement-system behaviour. Accepted trade-off: the present-tense seasonal copy can read slightly stale long after WWDC.
